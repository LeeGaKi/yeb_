<template>
  <div style="width: 500px">
    ...
    <el-tree :data="deps" :props="defaultProps" :filter-node-method="filterNode" :expand-on-click-node="false" ref="tree"
      ...
      <span class="custom-tree-node" slot-scope="{data }" style="display: flex;justify-content:space-between;width: 100%">
        <span style="margin-left: 30px">{{ data.name }}</span>
        <span>
          <el-button type="primary" class="depBtn" @click="() => showAddDep(data)">
            添加部门
          </el-button>
          <el-button type="danger" class="depBtn" @click="() => deleteDep(data)">
            删除部门
          </el-button>
        </span>
      </span>
